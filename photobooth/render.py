...
from photobooth import collage, QUALITY_FAST, QUALITY_SKEL, QUALITY_BEST
from photobooth.collage import Photo
...
class UserCollage(object):
    ...
    def make_page(self, opts):
        ...
        no_cols = opts.multi_col and int(round(math.sqrt(avg_ratio / ratio * virtual_no_imgs))) or math.ceil(len(self.photolist) / 2)

        self.page = collage.Page(opts.out_w, ratio, no_cols)
        random.shuffle(self.photolist)
        for photo in self.photolist:
            self.page.add_cell(photo, multi=opts.multi_col)
        self.page.adjust()
...
def create_collage(new_images, opts, callback):
    try:
        photolist = build_photolist(new_images)

        if len(photolist) > 0:
            new_collage = UserCollage(photolist)
            new_collage.make_page(opts)
            t = RenderingTask(
                page=new_collage.page,
                border_width=opts.border_w * max(new_collage.page.w,
                                                      new_collage.page.h),
